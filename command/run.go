...
func Run(args []string) error {
	...
	if mode == "server" {
		...
		log.Printf("ixa POC server listening on %s (TCP/TLS 1.3 + HTTP/3 upgrade)", cfg.Server.Address())
	...
	log.Printf("SOCKS5 listening on %s; ixa peer %s (TCP first, HTTP/3 upgrade)", cfg.SOCKS5.Address(), cfg.Peer.Address())
	if err := server.ListenAndServe(ctx); err != nil {
