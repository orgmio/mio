...
func TestTunnelRequestRoundTrip(t *testing.T) {
	...
	if err := writeRequest(&packet, key, tunnelCommandTCP, "example.com:443", now); err != nil {
		t.Fatal(err)
	}
	command, target, err := readRequest(&packet, key, now)
	if err != nil {
		t.Fatal(err)
	}
...
func TestClientHelloParserPreservesPartialInput(t *testing.T) {
	...
	for _, input := range inputs {
		preface, _, err := readClientRandom(bytes.NewReader(input))
		if err == nil {
...
func TestFallbackForwardsBytesUnchanged(t *testing.T) {
	certificate, err := tempCert("example.com")
	if err != nil {
		t.Fatal(err)
	}
...
func TestUDPFallbackForwardsDatagramsUnchanged(t *testing.T) {
	...
	if err := server.forwardUDP(context.Background(), writer, client, payload); err != nil {
		t.Fatal(err)
	}
	select {
	case got := <-writes:
...
func TestTLSAuthenticatedTunnelAndRelay(t *testing.T) {
	...
	server, err := newServer(
		...
		func(cover coverTarget) (tls.Certificate, error) {
			certificate, err := tempCert(cover.host)
			if err != nil {
			...
			certificate.PrivateKey, err = signKey(leaf)
	...
	authRandom, err := makeAuthRandom(server.key, "example.com", time.Now())
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := writeRequest(client, server.key, tunnelCommandTCP, "example.com:443", time.Now()); err != nil {
		t.Fatal(err)
	}
	...
	if !verifyServerRandom(server.key, "example.com", authRandom, serverRandom, time.Now()) {
...
func TestQUICAuthenticatedTunnelAndRelay(t *testing.T) {
	...
	go func() {
		...
		if acceptErr == nil {
			server.handleQUIC(serverContext, connection)
		}
	}()
...
func TestChromeAuthenticatedTCPTunnel(t *testing.T) {
	...
	server, err := newServer(
		TunnelServerConfig{Key: keyHex, SNI: "https://example.com:443"},
		func(cover coverTarget) (tls.Certificate, error) { return tempCert(cover.host) },
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	defer cancel()
	conn, err := client.openMuxStream(ctx, tunnelCommandTCP, "example.com:443")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAuthenticatedClientRandom(t *testing.T) {
	key := bytes.Repeat([]byte{0x42}, 32)
	now := time.Unix(1_800_000_000, 0)
	value, err := makeAuthRandom(key, "example.com", now)
	if err != nil {
		t.Fatal(err)
	}
	if !verifyAuthRandom(key, "example.com", value, now) {
		t.Fatal("valid ClientHello.Random was rejected")
	}
	if verifyAuthRandom(bytes.Repeat([]byte{0x43}, 32), "example.com", value, now) {
		t.Fatal("ClientHello.Random accepted with wrong key")
	}
	if verifyAuthRandom(key, "example.com", value, now.Add(time.Minute)) {
...
func TestAuthenticatedServerRandom(t *testing.T) {
	...
	now := time.Unix(1_800_000_000, 0)
	value, err := makeServerRandom(key, "example.com", clientRandom, now)
	if err != nil {
		t.Fatal(err)
	}
	if !verifyServerRandom(key, "example.com", clientRandom, value, now) {
		t.Fatal("valid ServerHello.Random was rejected")
	}
	wrongClientRandom := append([]byte(nil), clientRandom...)
	wrongClientRandom[0] ^= 1
	if verifyServerRandom(key, "example.com", wrongClientRandom, value, now) {
...
func TestParseCoverTarget(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.value, func(t *testing.T) {
			got, err := parseCover(test.value)
			if (err != nil) != test.wantErr {
				t.Fatalf("parseCover error = %v, wantErr %v", err, test.wantErr)
			}
			if got.host != test.wantHost || got.address != test.wantAddress {
				t.Fatalf("parseCover = %+v, want host=%q address=%q", got, test.wantHost, test.wantAddress)
			}
		})
	}
}
...
func TestTunnelRequestRejectsWrongKey(t *testing.T) {
	now := time.Unix(1_800_000_000, 0)
	var packet bytes.Buffer
	if err := writeRequest(&packet, bytes.Repeat([]byte{1}, 32), tunnelCommandTCP, "example.com:443", now); err != nil {
		t.Fatal(err)
	}
	_, _, err := readRequest(&packet, bytes.Repeat([]byte{2}, 32), now)
...
func TestTunnelRequestRejectsExpiredTimestamp(t *testing.T) {
	...
	var packet bytes.Buffer
	if err := writeRequest(&packet, key, tunnelCommandTCP, "example.com:443", time.Unix(100, 0)); err != nil {
		t.Fatal(err)
	}
	_, _, err := readRequest(&packet, key, time.Unix(200, 0))
