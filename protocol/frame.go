// 本文件定义 TLS 隧道建立完成后，客户端发送的首个认证帧格式。
//
// 为什么 ClientHello.Random 里的 HMAC 还不够：
//   - Random 字段是明文可见的，任何中间人都可以原样录制并重放整个 ClientHello。
//     单靠 Random 校验，等于服务端只验证了"这串字节是否曾经由持有 key 的人生成过"，
//     而无法阻止攻击者录制一次合法握手后无限期重放（在同一个时间窗口内始终有效）。
//   - 通过在 TLS 加密隧道建立之后，再传输一次绑定了本次连接语境的认证帧，
//     可以要求客户端证明自己确实持有 key 能够参与 TLS 密钥交换（因为这个帧
//     本身是在协商出的对称密钥下加密传输的，重放录制的密文在新连接的密钥下无法解密）。
//
// 帧格式（加密前的明文结构，长度前缀均为大端序）：
//
//	+------------+------------------+-------------------+
//	| version:1B | timestamp:8B(BE) | hmac_tag:32B       |
//	+------------+------------------+-------------------+
//
// hmac_tag = HMAC-SHA256(key, "ixa-go:auth-frame:v1" || version || timestamp)
package protocol
