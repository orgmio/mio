...
func (c *TunnelClient) openMuxStream(ctx context.Context, command byte, target string) (net.Conn, error) {
	for attempts := 0; attempts < 2; attempts++ {
		...
		if err := writeRequest(stream, c.key, command, target, time.Now()); err != nil {
			_ = stream.Close()
			c.dropTCPMux(session)
			continue
		}
